const DYNAMIC_STATES = new Set([1, 2]);
const LINE_LAYOUT = [
...
function createFlightImage(view) {
  ...
  const lines = view.text
    ...
    .map((line) => line.trim())
    .filter(Boolean)
    .slice(0, LINE_LAYOUT.length);
  if (!lines.length) return null;

  const text = lines
    .map((line, index) => {
      const layout = LINE_LAYOUT[index];
      return `<text x="98" y="${layout.y}" fill="${layout.color}" font-family="Arial, sans-serif" font-size="${layout.fontSize}" font-weight="${layout.fontWeight}" text-anchor="middle">${escapeXml(line)}</text>`;
    })
    .join("");
